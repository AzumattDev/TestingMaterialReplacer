...
namespace PieceManager
{
    ...
    public class SnapPointMaker
    {
        ...
        {
            foreach (GameObject? gameObject in _objectsToApplySnaps)
            {
        ...
        {
            Vector3[]? vertices = GetColliderVertexPosRotated(obj);
        ...
        {
            foreach (Vector3 VARIABLE in vector3S)
            {
                GameObject snappoint = new GameObject();
                snappoint.name = "_snappoint";
                snappoint.tag = "snappoint";
                snappoint.layer = 10;
                GameObject? temp = Object.Instantiate(snappoint, VARIABLE, Quaternion.identity, objecttosnap.transform);
